...
def generate_launch_description():
    ...
    iiwa_controllers = PathJoinSubstitution([FindPackageShare('wbc_ros'), 'config', 'performance_test', 'iiwa_controllers.yaml'])
    # Load the trajectory configuration file
    trajectory_publisher_config = os.path.join(get_package_share_directory('wbc_ros'),'config','performance_test','trajectory_publisher.yml')
    ...
    foot_fr_controller_spawner = Node(
        package='controller_manager',
        executable='spawner',
        arguments=['foot_fr_controller', '--controller-manager', ['/', 'controller_manager']],
        namespace='/')
    foot_fl_controller_spawner = Node(
        package='controller_manager',
        executable='spawner',
        arguments=['foot_fl_controller', '--controller-manager', ['/', 'controller_manager']],
        namespace='/')   
    foot_br_controller_spawner = Node(
        package='controller_manager',
        executable='spawner',
        arguments=['foot_br_controller', '--controller-manager', ['/', 'controller_manager']],
        namespace='/')      
    foot_bl_controller_spawner = Node(
        package='controller_manager',
        executable='spawner',
        arguments=['foot_bl_controller', '--controller-manager', ['/', 'controller_manager']],
        namespace='/')          
    ...
    delayed_foot_fr_controller_spawner = (
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=whole_body_controller_spawner,
                on_exit=[foot_fr_controller_spawner])))
    delayed_foot_fl_controller_spawner = (
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=foot_fr_controller_spawner,
                on_exit=[foot_fl_controller_spawner])))
    delayed_foot_br_controller_spawner = (
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=foot_fl_controller_spawner,
                on_exit=[foot_br_controller_spawner])))
    delayed_foot_bl_controller_spawner = (
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=foot_br_controller_spawner,
                on_exit=[foot_bl_controller_spawner])))
    delayed_joint_position_controller_spawner = (
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=foot_bl_controller_spawner,
